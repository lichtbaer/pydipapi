...
class TestAsyncDipAnfrage(unittest.TestCase):
    ...
    async def test_async_fetch_paginated_data(self):
        ...
        mock_response.json = AsyncMock(
            return_value={"documents": [{"id": 1}, {"id": 2}], "cursor": ""}
        )
        mock_response.status = 200
        mock_session.get.return_value.__aenter__.return_value = mock_response
    ...
    async def test_async_get_person(self):
        ...
        with patch.object(
            self.async_client, "_fetch_paginated_data", new_callable=AsyncMock
        ) as mock_fetch:
    ...
    async def test_async_get_person_ids(self):
        ...
        with patch.object(
            self.async_client, "_make_request", new_callable=AsyncMock
        ) as mock_request:
    ...
    async def test_async_get_documents_by_type(self):
        ...
        with patch.object(
            ...
            result = await self.async_client.get_documents_by_type(
                "kleine_anfrage", anzahl=5
            )
            self.assertEqual(len(result), 1)
    ...
    async def test_async_get_proceedings_by_type(self):
        ...
        with patch.object(
            ...
            result = await self.async_client.get_proceedings_by_type(
                "test_type", anzahl=5
            )
            self.assertEqual(len(result), 1)
